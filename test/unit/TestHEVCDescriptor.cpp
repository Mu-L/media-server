...
TEST(TestHEVCDescriptor, Serialize)
{
  Logger::EnableDebug(true);

  HEVCDescriptor descriptor;
  descriptor.configurationVersion = 1;
  descriptor.generalProfileSpace = 0;
  descriptor.generalTierFlag = 0;
  descriptor.generalProfileIdc = 1;
  descriptor.profileIndication = 1;
  descriptor.generalProfileCompatibilityFlags = 0x40000000;
  descriptor.generalConstraintIndicatorFlags = { 0x90, 0x00, 0x00, 0x00, 0x00, 0x00 };
  descriptor.generalLevelIdc = 123;
  descriptor.paddingBytes = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  descriptor.NALUnitLengthSizeMinus1 = 4;
  descriptor.numOfArrays = 3;
  descriptor.numOfVideoParameterSets = 1;
  // todo specify as vps data fields 
  const uint8_t vpsData[] = { 0x40, 0x01, 0x0c, 0x01, 0xff, 0xff, 0x01, 0x40, 
                              0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 
                              0x00, 0x00, 0x03, 0x00, 0x7b, 0x97, 0x02, 0x40 };
  Buffer vpsBuf(vpsData, sizeof(vpsData));
  descriptor.vpsData.push_back(std::move(vpsBuf));

  descriptor.numOfSequenceParameterSets = 1;
  // todo specify as sps data fields 
  const uint8_t spsData[] = { 0x42, 0x01, 0x01, 0x01, 0x40, 0x00, 0x00,
                              0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x7b, 0xa0, 0x03, 0xc0, 0x80, 0x11,
                              0x07, 0xcb, 0x96, 0x5d, 0x29, 0x08, 0x46, 0x45, 0xff, 0x8c, 0x05, 0xa8, 0x08, 0x08, 0x08, 0x20,
                              0x00, 0x00, 0x03, 0x00, 0x20, 0x00, 0x00, 0x07, 0x8c, 0x00, 0xbb, 0xca, 0x20, 0x00, 0x09, 0x89,
                              0x68, 0x00, 0x01, 0x31, 0x2d, 0x08 };
  Buffer spsBuf(spsData, sizeof(spsData));
  descriptor.spsData.push_back(std::move(spsBuf));

  descriptor.numOfPictureParameterSets = 1;
  // todo specify as pps data fields 
  const uint8_t ppsData[] = {0x44, 0x01, 0xc0, 0x93, 0x7c, 0x0c, 0xc9};
  Buffer ppsBuf(ppsData, sizeof(ppsData));
  descriptor.ppsData.push_back(std::move(ppsBuf));

  uint8_t buf[134] = {0};
  descriptor.Serialize(buf, 134);

  EXPECT_THAT(buf,
 				::testing::ElementsAreArray({
                0x01, 0x01, 0x40, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00, 0x00, 0x00, 0x7b, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0xfc, 0x03, 0x20, 0x00, 0x01, 0x00, 0x18, 0x40, 0x01, 0x0c, 0x01,
                0xff, 0xff, 0x01, 0x40, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
                0x7b, 0x97, 0x02, 0x40, 0x21, 0x00, 0x01, 0x00, 0x3d, 0x42, 0x01, 0x01, 0x01, 0x40, 0x00, 0x00,
                0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x7b, 0xa0, 0x03, 0xc0, 0x80, 0x11,
                0x07, 0xcb, 0x96, 0x5d, 0x29, 0x08, 0x46, 0x45, 0xff, 0x8c, 0x05, 0xa8, 0x08, 0x08, 0x08, 0x20,
